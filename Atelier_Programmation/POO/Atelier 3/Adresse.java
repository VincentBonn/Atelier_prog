...
public class Adresse {
	 ...
	 * Certaines v√©rifications sont fa√Ætes pour s'assurer de la
	...
	public final void  setCode_postal(String c_p){
		if (code_postal.equals(INCONNU_CHAINE)&&(c_p.matches("[0-9]{5}")))
			...
			if (!code_postal.equals(INCONNU_CHAINE))
				System.err.println("Le code postal a dÈj‡† ÈtÈ attribuÈ, vous ne pouvez plus le modifier...");
			else {
				System.err.println("Code postal mal Ècrit, il doit Ítre de la forme NNNNN avec N un chiffre diffÈrent de 0");
				System.err.println("Vous pouvez rÈessayer une nouvelle affectation...");
			}
		}
	}
	...
	public final void  setNumero(int n) {
		...
		else System.err.println("Numero incorrect, il doit Ítre positif...");
	}
	/**
